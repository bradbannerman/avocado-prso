...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 26 17:03:25 2020
...
def write_to_file(filename, myname, myage, major):
    ...
    age = str(myage)  # converts the age to a string.
...
def name_number(csv_filename):
    ...
    while True:
        nextname = input("Enter a Rangers player's name', press return to end: ")
        if nextname == "":
            break              # break jumps out of the loop
        nextnumber = input("Enter the player's squad number: ")
        print(nextname) 
        print(nextnumber)
